...
''' % (timeStamp,
       ...
       boardName,
       timeStamp,
       int(os.environ['PITON_NUM_CHIPS_SM']),
       int(os.environ['PITON_X_TILES']),
       int(os.environ['PITON_Y_TILES']),
       int(os.environ['PITON_NUM_TILES']) * int(os.environ['PITON_NUM_CHIPS_SM']),
       sysFreq,
...
/ {
    ...
    for i in range(len(devices)):
        if devices[i]["name"] == "mem":
            ...
            addrLen  = int(devices[i]["length"] / PITON_NUM_CHIPS_SM)
            for j in range(PITON_NUM_CHIPS_SM):
                tmpStr += '''
    memory%d@%08x {
        device_type = "memory";
        reg = <%s>;
        numa-node-id = <%d>;
            ...
            ''' % (j,addrBase, _reg_fmt(addrBase, addrLen, 2, 2),j)
                addrBase += addrLen
