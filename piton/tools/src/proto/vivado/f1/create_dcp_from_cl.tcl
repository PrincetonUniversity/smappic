...
#checking if PROTOSYN_RUNTIME_DESIGN_PATH env variable exists
if { [info exists ::env(PROTOSYN_RUNTIME_DESIGN_PATH)] } {
        set PROTOSYN_RUNTIME_DESIGN_PATH $::env(PROTOSYN_RUNTIME_DESIGN_PATH)
        puts "Using design directory $PROTOSYN_RUNTIME_DESIGN_PATH";
} else {
        puts "Error: PROTOSYN_RUNTIME_DESIGN_PATH environment variable not defined ! ";
        puts "This script must be run from protosyn!";
        exit 2
}

#checking if CL_BUILD_DIR env variable exists
if { [info exists ::env(CL_BUILD_DIR)] } {
        set CL_BUILD_DIR $::env(CL_BUILD_DIR)
        puts "Using CL directory $CL_BUILD_DIR";
} else {
        puts "Error: CL_BUILD_DIR environment variable not defined ! ";
        puts "Use export CL_BUILD_DIR=Your_Design_Root_Directory"
        exit 2
}
...
##################################################
set implDir   $CL_BUILD_DIR/checkpoints
set rptDir    $CL_BUILD_DIR/reports
set toAwsDir  $implDir/to_aws
...
puts "All reports and intermediate results will be time stamped with name \"$timestamp\"";
...
puts "AWS FPGA: ([clock format [clock seconds] -format %T]) Calling the setup.tcl.";
...
set scriptDir $DV_ROOT/tools/src/proto/vivado/f1
...
##################################################
if {${cl.synth}} {
   source -notrace $scriptDir/synth_${CL_MODULE}.tcl
   set synth_dcp ${timestamp}.CL.post_synth.dcp
} else {
   open_checkpoint $implDir/CL.post_synth.dcp
...
if {$implement} {
   ...
   if {$link} {
      ...
      add_files $implDir/$synth_dcp
      set_property SCOPED_TO_CELLS {WRAPPER_INST/CL} [get_files $implDir/$synth_dcp]

      #Read the constraints, note *DO NOT* read cl_clocks_aws (clocks originating from AWS shell)
      read_xdc [ list \
         $PROTOSYN_RUNTIME_DESIGN_PATH/f1/cl_pnr_user.xdc
      ]
      ...
      # write_checkpoint -force $implDir/${timestamp}.post_link.dcp
   }
   ...
   ##############################
   # Report final timing
   report_timing_summary -file $rptDir/${timestamp}.SH_CL_final_timing_summary.rpt
   ...
   write_checkpoint -force $implDir/${timestamp}.SH_CL_routed.dcp
   #checkpoint that will be sent to aws and hence encrypt
   write_checkpoint -encrypt -force $toAwsDir/${timestamp}.SH_CL_routed.dcp

   # Generate debug probes file
   write_debug_probes -force -no_partial_ltxfile -file $implDir/${timestamp}.debug_probes.ltx
...
set manifest_file [open "${toAwsDir}/${timestamp}.manifest.txt" w]
set hash [lindex [split [exec sha256sum $toAwsDir/${timestamp}.SH_CL_routed.dcp] ] 0]
set TOOL_VERSION $::env(VIVADO_TOOL_VERSION)
set vivado_version [string range [version -short] 0 5]
...
if { [file exists $toAwsDir/${timestamp}.Developer_CL.tar] } {
        puts "Deleting old tar file with same name.";
        file delete -force $toAwsDir/${timestamp}.Developer_CL.tar
}
